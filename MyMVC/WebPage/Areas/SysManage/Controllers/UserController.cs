...
namespace WebPage.Areas.SysManage.Controllers
{
    ...
    public class UserController : BaseController
    {
        ...
        /// 分页查询用户列表
        /// </summary>
        private PageInfo BindList(string DepartId)
        {
            //基础数据
        ...
        public ActionResult Detail(int? id)
        {
            try
            {
                ...
                if (id != null && id > 0)
                {
                    entity = this.UserManage.Get(p => p.ID == id);
                    postList = String.Join(",", entity.SYS_POST_USER.Select(p => p.FK_POSTID).ToList());
                }
                ViewBag.dpt = this.DepartmentManage.GetDepartmentByDetail();
        ...
        /// <returns></returns>
        public ActionResult SetUserInfo(SYS_USERINFO entity)
        {
            bool flag = false;
            var json = new JsonHelper() { Msg = "人员档案保存成功", Status = "n" };
            try
            {
                if (entity == null)
                {
                ...
                {
                    //entity.HomeTown = string.Concat(new string[]
                    //{
                    //    base.Request.Form["jgprov"],
                    //    ",",
                    //    base.Request.Form["jgcity"],
                    //    ",",
                    //    base.Request.Form["jgcountry"]
                    //});
                    //entity.HuJiSuoZaiDi = string.Concat(new string[]
                    //{
                    //    base.Request.Form["hkprov"],
                    //    ",",
                    //    base.Request.Form["hkcity"],
                    //    ",",
                    //    base.Request.Form["hkcountry"]
                    //});
                    if (entity.ID <= 0)
                    {
                    ...
                    if (this.UserInfoManage.SaveOrUpdate(entity, flag))
                    {
                        json.Status = "y";
                    }
                    else
