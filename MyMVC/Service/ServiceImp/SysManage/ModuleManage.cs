...
namespace Service.ServiceImp
{
    ...
    public class ModuleManage : RepositoryBase<Domain.SYS_MODULE>, IService.IModuleManage
    {
        ...
        /// <returns></returns>
        public List<SYS_MODULE> GetModuleBySysId(string sysId)
        {
            List<SYS_MODULE> result = new List<SYS_MODULE>();
            result = this.LoadListAll(p => sysId==p.FK_BELONGSYSTEM);
            //去重
            result = result.Distinct(new ModuleDistinct()).ToList();
            //返回模块集合
            return result.OrderBy(p => p.LEVELS).ThenBy(p => p.SHOWORDER).ToList();
        }
        /// <summary>
        /// 获取用户权限模块集合
        /// 
